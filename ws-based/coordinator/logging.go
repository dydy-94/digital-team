...
func logInfo(component, message string, fields ...interface{}) {
	entry := &LogEntry{
...
func logError(component, message string, fields ...interface{}) {
	entry := &LogEntry{
		Timestamp: time.Now().UTC().Format(time.RFC3339),
		Level:     "ERROR",
		Component: component,
...
func logWarn(component, message string, fields ...interface{}) {
	entry := &LogEntry{
		Timestamp: time.Now().UTC().Format(time.RFC3339),
		Level:     "WARN",
		Component: component,
...
func applyFields(entry *LogEntry, fields []interface{}) {
	for i := 0; i < len(fields); i += 2 {
		...
		case "agent_id":
			entry.AgentID, _ = fields[i+1].(string)
		case "msg_id":
